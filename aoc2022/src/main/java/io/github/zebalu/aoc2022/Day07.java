...
public class Day07 {
    ...
    public static void main(String[] args) {
        ...
        var dirs = readInput(root);
        part1(dirs);
        part2(root, dirs);
    }

    private static void part1(List<FSElement> dirs) {
        System.out.println(dirs.stream().filter(d -> d.size < DIR_LIMIT).mapToInt(d -> d.size).sum());
    }
    ...
    private static List<FSElement> readInput(FSElement root) {
        ...
        for (int index = 1; index < lines.size(); ++index) {
            current = processLine(current, lines.get(index));
        }
        fixSizes(root);
        return collectDirs(root);
    }

    private static FSElement processLine(FSElement current, String line) {
